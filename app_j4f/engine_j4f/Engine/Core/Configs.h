...
namespace engine {
    ...
    struct FpsLimit {
        FpsLimit() = default;
        explicit FpsLimit(const uint16_t limit, const FpsLimitType type = FpsLimitType::F_CPU_SLEEP) :
                fpsMax(limit), limitType(type) { }

        uint16_t fpsMax = 60u;
        FpsLimitType limitType = FpsLimitType::F_CPU_SLEEP;
    };

    struct EngineConfig {
        FpsLimit fpsLimitDraw;
        FpsLimit fpsLimitUpdate;
